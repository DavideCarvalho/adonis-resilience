...
<Callout type="info">
`@adonisjs/lucid`, `@adonisjs/redis` and `ioredis` are **optional peer dependencies** of `@agora/resilience`. Install the one your selected store needs — e.g. `npm i @adonisjs/redis ioredis` for `stores.redis()`.
</Callout>
...
```ts
import { SqlResilienceStore, type SqlDriver } from '@agora/resilience'
